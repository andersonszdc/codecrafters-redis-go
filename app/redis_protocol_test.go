...
func TestDecodeSimpleString(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error decoding simples string: %s", err)
	}
	...
	if value.String() != "foo" {
		t.Errorf("expected 'foo', got %v", value.String())
	}
}
...
func TestDecodeBulkString(t *testing.T) {
	...
	if value.String() != "abcd" {
		t.Errorf("expected 'abcd', got %v", value.String())
	}
}
...
func TestDecodeBulkStringArray(t *testing.T) {
	...
	if value.Array()[1].String() != "this" {
		t.Errorf("exptected 'this', got '%s'", value.Array()[1].String())
	}
}
